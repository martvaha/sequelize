...
class QueryGenerator {
  ...
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    if (this._dialect.supports.EXCEPTION && options.exception) {
      ...
      if (semver.gte(this.sequelize.options.databaseVersion, '9.2.0')) {
        ...
        valueQuery = `${`CREATE OR REPLACE FUNCTION pg_temp.testfunc(OUT response ${quotedTable}, OUT sequelize_caught_exception text) RETURNS RECORD AS ${delimiter}` +
          ' BEGIN '}${valueQuery} INTO response; EXCEPTION ${
          options.exception
        } END ${delimiter} LANGUAGE plpgsql; SELECT (testfunc.response).*, testfunc.sequelize_caught_exception FROM pg_temp.testfunc(); DROP FUNCTION IF EXISTS pg_temp.testfunc()`;
      } else {
        options.exception = 'WHEN unique_violation THEN NULL;';
        valueQuery = `CREATE OR REPLACE FUNCTION pg_temp.testfunc() RETURNS SETOF ${quotedTable} AS $body$ BEGIN RETURN QUERY ${valueQuery}; EXCEPTION ${
          options.exception
        } END; $body$ LANGUAGE plpgsql; SELECT * FROM pg_temp.testfunc(); DROP FUNCTION IF EXISTS pg_temp.testfunc();`;
      }
    }
    ...
    console.log('tempsetting', this._templateSettings);
  ...
  bulkInsertQuery(tableName, fieldValueHashes, options, fieldMappedAttributes) {
    ...
    if (this._dialect.supports.inserts.updateOnDuplicate && options.updateOnDuplicate) {
      onDuplicateKeyUpdate = ` ON DUPLICATE KEY UPDATE ${options.updateOnDuplicate
        .map(attr => {
    ...
    return `INSERT${ignoreDuplicates} INTO ${this.quoteTable(tableName)} (${attributes}) VALUES ${tuples.join(
      ','
    )}${onDuplicateKeyUpdate}${onConflictDoNothing}${returning};`;
  }
  ...
  arithmeticQuery(operator, tableName, attrValueHash, where, options, attributes) {
    ...
    return `UPDATE ${this.quoteTable(tableName)} SET ${values.join(',')}${outputFragment} ${this.whereQuery(
      where
    )} ${returningFragment}`.trim();
  }
  ...
  jsonPathExtractionQuery(column, path) {
    ...
    switch (this.dialect) {
      case 'mysql':
        ...
        quotedColumn = this.isIdentifierQuoted(column) ? column : this.quoteIdentifier(column);
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (options.group) {
      options.group = Array.isArray(options.group)
        ? options.group.map(t => this.quote(t, model)).join(', ')
        : this.quote(options.group, model);
      if (subQuery) {
    ...
    return (
      ...
      attributes.map(attr => {
        ...
        if (Array.isArray(attr)) {
          ...
          attr =
            !attr.includes(Utils.TICK_CHAR) && !attr.includes('"') ? this.quoteIdentifiers(attr) : this.escape(attr);
        }
    ...
    if (topLevelInfo.options.includeIgnoreAttributes !== false) {
      const includeAttributes = include.attributes.map(attr => {
        ...
        if (verbatim === true) {
          ...
          prefix = `json_extract(${this.quoteIdentifier(includeAs.internalAs)}.${attr.replace(
            /\(|\)|json_extract/g,
            ''
          )})`;
        } else {
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelInfo) {
    ...
    const throughAttributes = through.attributes.map(
      attr =>
        `${this.quoteIdentifier(throughAs)}.${this.quoteIdentifier(
          Array.isArray(attr) ? attr[0] : attr
        )} AS ${this.quoteIdentifier(`${externalThroughAs}.${Array.isArray(attr) ? attr[1] : attr}`)}`
    );
    const association = include.association;
  ...
  _whereParseSingleValueObject(key, field, prop, value, options) {
    ...
    switch (prop) {
      ...
      case Op.notBetween:
        return this._joinKeyValue(
          key,
          `${this.escape(value[0])} AND ${this.escape(value[1])}`,
          comparator,
          options.prefix
        );
      case Op.raw:
