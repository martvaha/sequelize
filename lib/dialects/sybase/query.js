...
const debug = logger.getLogger().debugContext('sql:sybase');
...
class Query extends AbstractQuery {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (_.startsWith(this.sql, 'BEGIN TRANSACTION')) {
        connection.beginTransaction(
          ...
          Utils.mapIsolationLevelStringToTedious(this.options.isolationLevel, connection.lib)
        );
        ...
        // exec teine argument on parameetrid
        connection.exec(...query, (err, results) => {
          //sybase  :autocommit mode
          ...
          //          console.log(`executed(${this.connection.uuid || 'default'}) : ${this.sql}`);
          ...
          if (err) {
            ...
            console.log(results);
            resolve(this.formatResults(results, results.length));
          } else {
            resolve();
          }
        });
  ...
  run(sql, parameters) {
    console.log('run-sql', sql, parameters);
  ...
  */
  ...
  formatError(err) {
    ...
    match = err.message.match(
      /Violation of UNIQUE KEY constraint '((.|\s)*)'. Cannot insert duplicate key in object '.*'.(:? The duplicate key value is \((.*)\).)?/
    );
    ...
    match =
      ...
      err.message.match(
        /The MERGE statement conflicted with the FOREIGN KEY constraint "(.*)". The conflict occurred in database "(.*)", table "(.*)", column '(.*)'./
      ) ||
      err.message.match(
        /The UPDATE statement conflicted with the FOREIGN KEY constraint "(.*)". The conflict occurred in database "(.*)", table "(.*)", column '(.*)'./
      );
  ...
  isShowOrDescribeQuery() {
    ...
    result =
      ...
      this.sql
        .toLowerCase()
        .indexOf("select c.column_name as 'name', c.data_type as 'type', c.is_nullable as 'isnull'") === 0;
  ...
  handleInsertQuery(results, metaData) {
    if (this.instance) {
      ...
      if (
        this.model.rawAttributes.hasOwnProperty(autoIncrementAttribute) &&
