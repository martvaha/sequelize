  ...
  constructor(database, username, password, options) {
    ...
    switch (this.getDialect()) {
      ...
      case 'sqlite':
        ...
        break;
      case 'sybase':
        Dialect = require('./dialects/sybase');
        break;
      default:
        throw new Error(
          `The dialect ${this.getDialect()} is not supported. Supported dialects: mssql, mysql, postgres, and sqlite.`
        );
    }
        ...
        retryParameters =>
          Promise.try(() => {
            ...
            if (options.bind) {
              ...
              console.log('bindParameters', bindParameters);
            }
            ...
            if (options.transaction && options.transaction.finished) {
              const error = new Error(
                `${options.transaction.finished} has been called on this transaction(${
                  options.transaction.id
                }), you can no longer use it. (The rejected query is attached as the \'sql\' property of this error)`
              );
              error.sql = sql;
            ...
            return query.run(sql, bindParameters).finally(() => {
    ...
    if (options.match) {
      if (!options.match.test(this.config.database)) {
        return Promise.reject(
          new Error(`Database "${this.config.database}" does not match sync match parameter "${options.match}"`)
        );
      }
    }
